...
class WSFECred(BaseWS):
    ...
    def __analizar_errores(self, ret):
        ...
        self.Errores = [err['codigoDescripcion']
                        for err in ret.get('arrayErrores', [])]
        self.ErroresFormato = [err['codigoDescripcionString']
                               for err in ret.get('arrayErroresFormato', [])]
    ...
    def __analizar_observaciones(self, ret):
        ...
        self.Observaciones = [obs["codigoDescripcion"]
                              for obs in ret.get('arrayObservaciones', [])]
        self.Obs = '\n'.join(["%(codigo)s: %(descripcion)s" %
                              obs for obs in self.Observaciones])
    ...
    def ConsultarTiposAjustesOperacion(self, sep="||"):
        ...
        # para informar la aceptación de una Factura Electrónica de Crédito y
        # su Cuenta Corriente vinculada
    ...
    def ConsultarTiposRetenciones(self, sep="||"):
        ...
        return [("%s {codigoJurisdiccion} %s {descripcionJurisdiccion} %s {porcentajeRetencion} %s" %
...
if __name__ == '__main__':
    ...
    try:
        ...
        wsaa = WSAA()
        ta = wsaa.Autenticar("wsfecred", CERT, PRIVATEKEY,
                             wsaa_url, debug=DEBUG)
        if not ta:
        ...
        if '--obligado' in sys.argv:
            try:
                cuit_consultar = int(
                    sys.argv[sys.argv.index("--obligado") + 1])
            ...
            reg = {"obligado": [
                {"resultado": wsfecred.Resultado, "monto_desde": ret}]}
        ...
        if '--comprobantes' in sys.argv:
            ...
            try:
                ...
                cuit_contraparte = int(
                    sys.argv[sys.argv.index("--comprobantes") + 4])
            except IndexError as ValueError:
                pass
        ...
        if '--prueba' in sys.argv:
            ...
            fec['retenciones'] = [
                dict(cod_tipo=1, desc_motivo="Ret Prueba", importe=1000, porcentaje=1.00)]
            fec['ajuste_operacion'] = [dict(codigo=1, importe=57.00)]
            fec['confirmar_nota_dc'] = [dict(
                cuit_emisor=30999999999, tipo_cbte=202, punto_vta=99, nro_cbte=1, acepta="S")]
            fec['motivo_rechazo'] = [
